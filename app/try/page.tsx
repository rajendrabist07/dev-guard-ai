...
export default function TryPage() {
  ...
  const [historyRuns, setHistoryRuns] = useState<TryRun[]>([]);
...
+  const res = await fetch('/api/log');`
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      sid = localStorage.getItem('devguard_try_session_id') || '';
      if (!sid) {
        sid = `sess_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
        localStorage.setItem('devguard_try_session_id', sid);
      }
      setSessionId(sid);
    }
  }, []);
  ...
  const loadHistory = useCallback(async (targetSessionId?: string) => {
    ...
    try {
      const res = await fetch(`/api/try/history?sessionId=${encodeURIComponent(sid)}`);
            ...
            try {
              ...
              if (data.type === 'progress') {
                ...
                setResult({
                  ...
                  shareUrl: data.shareUrl || `/try/result/${data.reviewRunId}`,
                });
  ...
  const copyShareLink = (runId: string) => {
    if (typeof window !== 'undefined') {
      const url = `${window.location.origin}/try/result/${runId}`;
  ...
  const copyHistoryLink = (runId: string) => {
    if (typeof window !== 'undefined') {
      const url = `${window.location.origin}/try/result/${runId}`;
      navigator.clipboard.writeText(url);
      setHistoryCopiedId(runId);
      setTimeout(() => setHistoryCopiedId(null), 2000);
    }
  };

  const getTopSeverity = (findings: Finding[]): Severity | 'clean' => {
  ...
  const getSeverityBadge = (severity: Severity | 'clean' | string) => {
    switch (severity) {
      case 'critical':
        return (
      ...
      case 'clean':
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/10 border border-emerald-500/30 text-emerald-300">
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0b0f19] text-gray-100 selection:bg-emerald-500/30 selection:text-emerald-300">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-6 space-y-4">
              ...
              <div>
                ...
                />
                <div className="flex items-center space-x-2 text-[11px] text-gray-400 mt-2 font-mono">
                  <Lock className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                  <span>
                    Don&apos;t paste proprietary or sensitive credentials here. All test results generate public shareable links.
                  </span>
                </div>
              </div>
        ...
        {/* History Section: Past Test Runs in this browser */}
        <div className="space-y-4 pt-6 border-t border-gray-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <History className="w-5 h-5 text-emerald-400" />
              <h2 className="text-lg font-bold text-white tracking-tight">Your Past Test Runs</h2>
            </div>
            <button
              onClick={() => loadHistory()}
              disabled={loadingHistory}
              className="text-xs font-semibold text-gray-400 hover:text-emerald-300 flex items-center space-x-1.5 transition-colors"
            >
          ...
          <div className="bg-gray-900/60 border border-gray-800 rounded-2xl overflow-hidden">
            {historyRuns.length === 0 ? (
              <div className="p-8 text-center space-y-3">
                ...
                <div className="text-sm font-semibold text-gray-300">No test runs recorded in this session yet</div>
              ...
              <div className="divide-y divide-gray-800/80">
                {historyRuns.map((run) => (
                  <div
                    key={run.id}
                    className="p-4 sm:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-gray-800/30 transition-colors"
                  >
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 uppercase">
                          {run.input_type === 'sample' ? 'Sample File' : 'Custom Code'}
                        </span>
                        {getSeverityBadge(getTopSeverity(run.findings))}
                      ...
                      <h3 className="text-sm font-bold text-white hover:text-emerald-300 transition-colors">
                        <Link href={`/try/result/${run.id}`}>{run.pr_title}</Link>
                      </h3>
                    ...
                    <div className="flex items-center gap-2.5 self-start sm:self-center">
                      <button
                        onClick={() => copyHistoryLink(run.id)}
                        className="px-3 py-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold transition-colors flex items-center space-x-1.5"
                      >
                        {historyCopiedId === run.id ? (
                          <>
                            <Check className="w-3.5 h-3.5 text-emerald-400" />
                            <span>Copied!</span>
                          </>
                      ...
                      <Link
                        href={`/try/result/${run.id}`}
                        className="px-3.5 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black font-bold text-xs transition-all flex items-center space-x-1"
                      >
